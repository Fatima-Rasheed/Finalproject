...
export default function StudentMessagesPage() {
  ...
  const [recipientType, setRecipientType] = useState<'supervisors' | 'admin'>('supervisors');
  ...
  useEffect(() => {
    ...
    const fetchAdmin = async () => {
      ...
      const list = snapshot.docs.map((doc) => doc.data().email);
      if (list.length > 0) setAdminEmail(list[0]);
    };
      ...
      if (user?.email) {
        ...
        const unsubscribeMessages = onSnapshot(q, (snapshot) => {
          const msgs = snapshot.docs
            ...
            .filter((msg: any) => {
              if (typeof msg.to !== 'string' || typeof msg.from !== 'string') return false;
              return msg.to === user.email || msg.from === user.email;
            });
  ...
  const currentList =
    recipientType === 'admin' && adminEmail
  ...
  const handleSendMessage = async () => {
    ...
    const sender = user.email?.trim().toLowerCase();
    const receiver = selectedRecipient.trim().toLowerCase();
  ...
  return (
    <div className="flex flex-col flex-1 md:ml-64">
      ...
      <main className="pt-20 p-6 bg-gray-100 min-h-screen">
        ...
        {/* Main layout: left send, right messages */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {/* Message History */}
          <div className="bg-white p-6 rounded shadow max-h-[550px] overflow-y-auto">
            <h2 className="text-2xl font-bold mb-4">Your Messages</h2>
            {receivedMessages.length === 0 ? (
              <p className="text-gray-500">No messages received yet.</p>
            ) : (
              receivedMessages.map((msg) => (
                <div key={msg.id} className="mb-4 p-4 bg-gray-100 rounded-md shadow-sm">
                  <p>
                    <strong>From:</strong> {msg.from === userEmail ? 'You' : (msg.fromName || msg.from)}
                  </p>
                  <p>
